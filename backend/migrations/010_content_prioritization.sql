...
-- Add priority to impact_stories
ALTER TABLE impact_stories ADD COLUMN IF NOT EXISTS priority INTEGER DEFAULT 0;
CREATE INDEX IF NOT EXISTS idx_impact_stories_priority ON impact_stories(priority DESC, created_at DESC) WHERE status = 'published';
